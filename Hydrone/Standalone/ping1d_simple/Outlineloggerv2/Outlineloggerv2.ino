...
void setup() {
  ...
  String dataString = "";
  dataString += "lat,lon,depth,confidence,time";
...
void loop() {
  ...
  // Ping 1d loop
  if (ping.update()) {
    // Uncomment these lines to print distance and confidence
  ...
  if (myGNSS.getPVT() == true) {
    // Uncomment these lines to get latitude, longitude, and altitude
  ...
  // Read and append latitude and longitude
  dataString += String(myGNSS.getLatitude());
  dataString += ",";
  dataString += String(myGNSS.getLongitude());
  dataString += ",";

  // Append distance and confidence
  dataString += String(ping.distance() * .001);
  dataString += ",";
  dataString += String(ping.confidence());
  dataString += ",";

  // Get time with milliseconds and format it correctly
  int hour = myGNSS.getHour();
  int minute = myGNSS.getMinute();
  int second = myGNSS.getSecond();
  int millisecond = myGNSS.getMillisecond();

  char timeBuffer[20];
  sprintf(timeBuffer, "%02d:%02d:%02d.%03d", hour, minute, second, millisecond);
