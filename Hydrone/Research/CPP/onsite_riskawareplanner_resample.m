...
shallowcut=[-.1,.1]; %pdf intigration region
...
%lat,lon,depth,confidence,time

refoutline=readtable("TestData\Data\test26\surveyB1_high.txt");
...
%% ArdinoSURVEYDATA ERRRRRRRR
outline=readtable("TestData\Test_25\DATALOG1.txt");
...
figure;
hold on
plot(refoutline.GPSLongitude,refoutline.GPSLatitude)
plot(long0,lat0)
hold off
...
enu = lla2enu([refoutline.GPSLatitude,refoutline.GPSLongitude, zeros(size(refoutline.GPSLongitude))],[refLat, refLon, refAlt], 'ellipsoid');
easting1= enu(:,1);
northing1= enu(:,2);
alt1=enu(:,3);
...
rpoints = R*[easting1';northing1'];
easting1 = rpoints(1,:)' - eastingoffset;
northing1 = rpoints(2,:)'-northingoffset;
...
for i = 1:size(wptsMin,1)-1
    ...
    lineX = [wptsMin(i,1), newpoint(1,1)]; % X-coordinates of the line
    lineY = [wptsMin(i,2), newpoint(1,2)]; % Y-coordinates of the line
    [intersectX, intersectY] = polyxpoly(lineX, lineY, BuffObstical{1,ii}.Vertices(:,1),BuffObstical{1,ii}.Vertices(:,2));
    if move>=10
...
if numel(ObsticalVertices) > 0 && numel(BuffObstical) > 0
...
title({sprintf('Iteration: %.0f, Buffer size: %.3f', counter, Buff), 'Path Plan'},'Fontsize',14);
axis equal
...
waypoints=[wptsMin(:,1),wptsMin(:,2)]; %% change this lolllolol
...
[collisionChance] = simusvriskcalculation(X,Y,z,zcov,inter,interwayx,interwayy,sig,cellnum, shallowcut, ObsticalVertices, polyroi.Vertices,counter,Buff);
...
xlabel('X (m)','Fontsize',12);
ylabel('Y (m)','Fontsize',12);
ylabel(colorbar,'Probability','Fontsize',12)
title({sprintf('Iteration: %.0f, Buffer size: %.3f', counter, Buff), 'Collision Risk'},'Fontsize',14);
axis equal
...
% % filename = 'Test25_waypoints.waypoints';
% % 
...
%% Save items to .mat for analysis


% % save('Testprac_data.mat',"costj","buffer","rotationangle","riskobby","ObsticalVertices","polyroi","WGS1984")
...
subplot(2,1,2)
hold on
plot(minutes*2,'b',LineWidth=2)
scatter(1:size(minutes,2),minutes*2,100,'b',Marker="+",LineWidth=2)
...
%% ROI data, previous and current testing

figure

 
hold on
plot3(roiVertices(:,1),roiVertices(:,2),10*ones(size(roiVertices)),'g',Linewidth=2)
plot3(ObsticalVertices{1,1}(:,1),ObsticalVertices{1,1}(:,2),10*ones(size(ObsticalVertices{1,1})),'r',Linewidth=2)
% plot3(riskobby{1,1}.Vertices(:,1),riskobby{1,1}.Vertices(:,2),10*ones(size(riskobby{1,1}.Vertices)),'b',Linewidth=2)
...
%plot3(easting,northing,10*ones(size(easting)),'k',linewidth=2)
% plot3(easting1,northing1,10*ones(size(easting1)),'k',linewidth=1,linestyle='--')

axis equal
ylim([90,165])
xlim([50,155])
title('ROI Survey',Fontsize=14)
xlabel('X (m)',fontsize=12)
ylabel('Y (m)',fontsize=12)
ylabel(colorbar,'Depth (m)',fontsize=12)

%legend('GNSS trace','Bent Obstacle','Risk Obstacle')



%% Workspace Development

% plot(refoutline.GPSLatitude,refoutline.GPSLongitude)
% figure
% scatter(WGS1984(:,2),WGS1984(:,1))
% hold on
% plot(refoutline.GPSLongitude,refoutline.GPSLatitude)
% buffriskobby = polybuffer(riskobby{1,1},1.41)
