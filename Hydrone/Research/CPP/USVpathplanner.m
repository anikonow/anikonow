...
%init
cellnum=20; %cells per interpolated point
shallowcut=[-.1,.5]; %pdf intigration region
...
for ii = 1:1:obnum+1%,obnum+1:-1:1,1:1:obnum+1]
...
xlabel('X (m)')
ylabel('Y (m)')
title(sprintf('Path plan, Buffer size: %d', Buff));
hold off
...
[sig,interwayx,interwayy,inter,ang ] = sigmainterpol(waypoints);
