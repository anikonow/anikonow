...
shallowcut=[-.1,.4]; %pdf intigration region
...
B = 5 * (perlinNoise((X1+50)/50 , (Y1+50)/50 ) - 0.5)+3;



% % figure;
% % hold on;
% % surf(X1, Y1, B, 'EdgeColor', 'none'); % Surface plot
% % colormap turbo;
% % colorbar;
% % xlabel('X (m)');
% % ylabel('Y (m)');
% % zlabel('Depth (m)');
% % title('Survey Simulation: Click to Collect Points');
% % view(2); 
% % grid on;
% % axis equal
% % 
% % disp('Click to collect depth points. Press Enter to finish.');
% % [x_click, y_click] = ginput; 
% % 
% % z_click = interp2(X1, Y1, B, x_click, y_click);
% % 
% % scatter3(x_click, y_click, z_click, 50, 'r', 'filled');
% % 
% % legend('Depth Surface', 'Collected Points');
% % hold off;
...
% outline=readtable("hydrone\TestData\Test_21\Test21_outline.csv");
...
if obnum > 0
    ...
    plot (easting, northing,color='r',linewidth=2)
    plot (roiVertices(:,1),roiVertices(:,2),color='k',Linewidth=2)
...
% 
% Xq = [X(:), Y(:)]; 
data = [inter(:,1),inter(:,2)]; 
Z1=interp2(X1,Y1,B,inter(:,1),inter(:,2));

Xq = [X1(:), Y1(:)]; 
gprMdl = fitrgp(data, Z1, 'KernelFunction', 'squaredexponential');

[z1,zcov1] = predict(gprMdl, Xq);
z1 = reshape(z1, size(X1));
zcov1 = reshape(zcov1, size(X1));


depthrmse(counter)=sum(sum(sqrt(mean(B-z1).^2)/1500));
...
title("Coverage Plan",'Fontsize',14)
...
title("Buffer size",'Fontsize',14)
xlabel('Iteration','Fontsize',12)
ylabel('Value','Fontsize',12)
legend('Buffer size (m)','Risk','Target','Fontsize',10)
hold off
...
title("Estimated Survey Time",'Fontsize',14)
xlabel('Iteration','Fontsize',12)
ylabel('Value','Fontsize',12)
legend('Time (min)','Fontsize',10)
hold off


figure;
hold on
plot(depthrmse,'k',LineWidth=2)
scatter(1:size(depthrmse,2),depthrmse,100,'k',Marker="+",LineWidth=2)
% plot(varrmse,'r',LineWidth=2)
% scatter(1:size(depthrmse,2),varrmse,100,'r',Marker="+",LineWidth=2)
title("Average RMS",'Fontsize',14)
xlabel('Iteration','Fontsize',12)
ylabel('RMS (m)','Fontsize',12)
legend('Depth RMS','Fontsize',10)
hold off
...
function noise = perlinNoise(X, Y)
    noise = interp2(rand(10), X, Y, 'cubic', 0); 
end