...
survey=readtable("TestData\Test_8\test8_survey.txt");
...
survey.DOL= abs(survey.DOL)./3.281;
...
for k=1:length(outline.GPSLongitude)-1
    plot(outline.GPSLatitude(k:k+1),outline.GPSLongitude(k:k+1),'Color',z1_color(k,:),"linewidth",3)
    %fprintf('loop ran %f\n',k)
end
scatter(path.Var3,path.Var2,'r',"linewidth",1,"Marker","+")
colorbar;
