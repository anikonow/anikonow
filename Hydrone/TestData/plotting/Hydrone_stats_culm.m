...
scatter(culm.surveytime,culm.portloss,'+',"blue",linewidth=2)
plot(portx,porty,"color","b",linewidth=2)

scatter(culm.surveytime,culm.stbdloss,'+',"red",linewidth=2)
plot(stbdx,stbdy,"color","r",linewidth=2)

xlabel("Survey Duration (mins)",fontsize=12)
ylabel("Battery Drain %",fontsize=12)

ylim([0,max(culm.portloss)+5]);
legend("Port battery","Port LoBF","STBD battery","STBD LoBF",fontsize=12)
title("Battery Loss vs Survey Duration",fontsize=14)

i = 1;
%setuptime vs test id (show team size in color) test 5 3 people
figure;
hold on
scatter(culm.ID(i),culm.setuptime(i),"red","marker","o","LineWidth",2);
scatter(culm.ID(i),culm.setuptime(i),"green","marker","o","LineWidth",2);
scatter(culm.ID(i),culm.setuptime(i),"blue","marker","o","LineWidth",2);
...
ylabel("Packing Time (min)",fontsize=12)
xlabel("Test ID",fontsize=12)
title("Setup Time",fontsize=14)
legend('3 Surveyor','2 Surveyor','1 Surveyor',fontsize=12)
hold off
